...
from pipeline.models import LLMExtractionResult
...
INSERT_ACTION_ITEMS = """
    ...
    """
UPDATE_MEETING_SUMMARY = """UPDATE meetings SET summary = ? WHERE meeting_id = ?"""
DELETE_ACTION_ITEMS = """DELETE FROM action_items WHERE meeting_id = ?"""
...
def _fetch_utterances(meeting_id: str) -> list[dict]:
    conn = get_connection()
    rows = conn.execute(SELECT_UTTERANCES, (meeting_id,)).fetchall()
    conn.close()
...
def _log_attempt(meeting_id: str, attempt: int, success: bool,
    ...
    conn = get_connection()
    with conn:
        conn.execute(INSERT_LLM_EXTRACTION_LOGS, (meeting_id, attempt, MODEL, int(success), error_message, raw_response))
    conn.close()
...
def _save_result(result: LLMExtractionResult):
    ...
    with conn:
        # 회의 요약 업데이트
        conn.execute(UPDATE_MEETING_SUMMARY, (result.meeting_summary, result.meeting_id))

        # 기존 액션아이템 삭제 후 재적재 (멱등)
        conn.execute(DELETE_ACTION_ITEMS, (result.meeting_id,))

        for item in result.action_items:
            action_id = f"act_{uuid.uuid4().hex[:10]}"
            conn.execute(INSERT_ACTION_ITEMS, (
...
def extract(meeting_id: str, advertiser: str) -> LLMExtractionResult | None:
    """
